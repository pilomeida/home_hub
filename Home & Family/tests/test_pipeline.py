...
async def test_ingest_document_marks_needs_attention_on_unexpected_extraction_error(session, monkeypatch, tmp_path):
    """extract_bill/ensure_image can raise errors beyond ExtractionError/OSError
    (Anthropic SDK errors, pdf2image-specific errors on a corrupt PDF, etc.).
    These must still land the Document on needs_attention, not propagate uncaught."""
    document = Document(
        filename="corrupt.pdf", file_path=str(tmp_path / "corrupt.pdf"), content_hash="hash3",
        source=DocumentSource.MANUAL, status=DocumentStatus.PENDING,
    )
    session.add(document)
    session.commit()
...
async def test_ingest_document_marks_needs_attention_on_enrichment_failure(session, monkeypatch, tmp_path):
    """If enrichment (todo generation / wiki assessment) fails after the
    Transaction has already been committed, the Document must move to
    needs_attention rather than being stuck at PENDING — and the Transaction
    that was already committed must remain persisted."""
    document = Document(
        filename="bill.pdf", file_path=str(tmp_path / "bill.pdf"), content_hash="hash4",
        source=DocumentSource.MANUAL, status=DocumentStatus.PENDING,
    )
    session.add(document)
    session.commit()

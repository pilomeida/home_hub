...
async def create_vision_session(
    ...
    try:
        ...
        print(f"[vision] session {sid} fatal error: {exc}", flush=True)
        # Try to load the placeholder session (saved by start_import) and mark it failed
        try:
            session = load_session(sid)
        except Exception:
            session = PdfIngestionSession(
                session_id=sid, pdf_path=pdf_path, book_title=book_title,
                book_slug=book_slug, all_recipes=[], sampled_indices=[],
                extracted={}, extraction_complete=False,
                pipeline="vision", test_mode=test_mode,
            )
